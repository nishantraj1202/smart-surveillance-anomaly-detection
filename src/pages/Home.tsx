...
import { VideoUpload } from "@/features/video-analysis/components/VideoUploader";
import { ProcessingStatus } from "@/features/video-analysis/components/AnalysisProgress";
import { ResultsCard, AnalysisResult } from "@/features/video-analysis/components/ThreatAnalysisResult";
import { extractFramesFromVideo, ExtractedFrame } from "@/features/video-analysis/utils/frameExtractor";
import { supabase } from "@/core/api/client";
import { useToast } from "@/core/hooks/use-toast";

const Index = () => {
  const [analysisStage, setAnalysisStage] = useState<"idle" | "extracting" | "analyzing">("idle");
  ...
  const isProcessing = analysisStage !== "idle";
    ...
    <div className="flex min-h-screen flex-col bg-background">
      ...
      <main className="relative z-10 flex flex-1 flex-col items-center px-6 py-12">
        <div className="w-full max-w-3xl space-y-8">
          ...
          {/* Upload */}
          {!analysisResult && (
            <VideoUpload onFileSelect={handleVideoUpload} isProcessing={isProcessing} />
          )}

          {/* Processing */}
          <AnimatePresence>
            {isProcessing && (
              <ProcessingStatus analysisStage={analysisStage} progress={progress} />
            )}
          </AnimatePresence>
          ...
          <AnimatePresence>
            {analysisResult && (
              <motion.div
                ...
                className="space-y-4"
              >
                <ResultsCard analysisResult={analysisResult} frameThumbnails={videoFrames} />
                <motion.button
